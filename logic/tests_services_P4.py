...
class GetMoveServiceTests(tests_services.PlayGameBaseServiceTests):
    def setUp(self):
        ...
        self.game = Game.objects.create(
            cat_user=self.user1, mouse_user=self.user2,
        ...
        for move in self.moves:
            Move.objects.create(
                game=self.game, player=move["player"], origin=move["origin"],
                target=move["target"])
    ...
    def test1(self):
        ...
        response = self.client.get(reverse(GET_MOVE_SERVICE), {"shift": 1},
                                   follow=True)
    ...
    def test2(self):
        ...
        for move in self.moves:
            response = self.client1.post(
                reverse(GET_MOVE_SERVICE), {"shift": 1}, follow=True,
